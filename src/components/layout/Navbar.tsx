    ...
    <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-slate-950/80 backdrop-blur">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#A2D729]/90">
              Atlantic Pathway
            </p>
        ...
        <nav className="hidden flex-wrap gap-4 text-sm text-slate-200 sm:flex">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="transition hover:text-[#A2D729]"
            >
              {item.label}
            </a>
          ))}
        ...
        <button
          ...
          className="flex items-center justify-center rounded-full border border-white/10 p-2 text-white transition hover:border-[#A2D729]/60 sm:hidden"
